...
🚨 ALERT — AfriGov Sentinel-inspired 8-second dispatch
      ...
      └── Auto-tweet triggered if enabled
      │
      ▼
...
| Hackathon Winner | What We Borrowed |
|---|---|
| **PHAROS** (pharmacovigilance AI) | Signal detection engine — detect danger before it becomes a disaster |
| **AfriGov Sentinel** | 8-second alert dispatch — right alert, right person, instantly |
| **Metal Birds Watch** | Core principle — "let the agent watch, so you don't have to" |
...
│   ├── tinyfish.py      ← GitHub Issues agent + file cache
│   ├── competitor.py    ← Top-1 competitor benchmark (API + TinyFish hybrid)
│   ├── watchlist.py     ← Autonomous monitor + signal dispatch
│   ├── signals.py       ← PHAROS-style detection engine
│   ├── autopost.py      ← Twitter browser automation (in progress)
│   ├── start.bat        ← One-click server start with keys
│   └── requirements.txt
└── frontend/
    └── App.jsx          ← React dashboard with workflow banner
